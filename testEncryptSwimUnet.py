...
parser.add_argument('--outlogs', default='./testing/',
...
parser.add_argument('--test', default='/share/home/wangzy/PycharmProjects/data/val/', help='test mode, you need give the test pics dirs in this param')
...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader, 0):
            ...
            cover_img = all_pics[0:this_batch_size, :, :, :]  # batchSize,3,256,256
            secret_img = all_pics[this_batch_size:this_batch_size * 2, :, :, :]
            encryption_begin=time.time()
            secret_encryption_img = new_Encryption.encryption(secret_img)
            # secret_img = torch.unsqueeze(secret_img, dim=0)
            encryption_end = time.time()
            # print(encryption_end-encryption_begin)
            # concat cover and original secret to get the concat_img with 6 channels
            concat_img = torch.cat([cover_img, secret_encryption_img], dim=1)
            ...
            Hnet_begin=time.time()
            container_img = Hnet(concat_imgv)  # take concat_img as input of H-net and get the container_img
            Hnet_end = time.time()
            # print(Hnet_end-Hnet_begin)
            errH = criterion(container_img, cover_imgv)  # H-net reconstructed error
            Hlosses.update(errH.item(), this_batch_size)

            Rnet_begin = time.time()
            rev_secret_img_ori = Rnet(container_img)  # containerImg as input of R-net and get "rev_secret_img"
            Rnet_end = time.time()
            # print( Rnet_end-Rnet_begin)
            secret_imgv = Variable(secret_img, volatile=True)  # secret_imgv as label of R-net
            # rev_secret_img = Encryption.encryption(secret_encryption_img, x)
            unencryption_begin=time.time()
            rev_secret_img = new_Encryption.unencryption(rev_secret_img_ori)
            unencryption_end = time.time()
            # print(unencryption_end-unencryption_begin)

            rev_secret_img=rev_secret_img.cuda()
            errR = criterion(rev_secret_img, secret_imgv)  # R-net reconstructed error
            Rlosses.update(errR.item(), this_batch_size)

            # rev_secret_img = Encryption.encryption(rev_secret_img, x)
            ...
            # rev_secret_img = Encryption.encryption(rev_secret_img)
            # rev_secret_img = rev_secret_img.cuda()
            ...
            save_result_pic(this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img, epoch, i,
                            opt.testPics)
