...
DATA_DIR = '/share/home/wangzy/PycharmProjects/data/'
...
def main():
    ...
    if opt.test == '':
        ...
        # opt.Hnet = "/share/home/wangzy/PycharmProjects/PyTorch-Deep-Image-Steganography/PyTorch-Deep-Image-Steganography-master/training/c05_2021-11-18-17_28_42/checkPoints/netH_epoch_3,sumloss=0.001597,Hloss=0.000451.pth"
        # opt.Rnet = "/share/home/wangzy/PycharmProjects/PyTorch-Deep-Image-Steganography/PyTorch-Deep-Image-Steganography-master/training/c05_2021-11-18-17_28_42/checkPoints/netR_epoch_3,sumloss=0.001597,Rloss=0.002292.pth"
        testdir = opt.test
    ...
    Hnet =  SwinIR(upscale=1, in_chans=9, img_size=144, window_size=8,
    ...
    Rnet = SwinIR(upscale=1, in_chans=3, out_chans=6, img_size=144, window_size=8,
...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    test_results['psnr'] = []
    test_results['ssim'] = []
    test_results['psnr_secret1'] = []
    test_results['ssim_secret1'] = []
    test_results['psnr_secret2'] = []
    test_results['ssim_secret2'] = []
    with torch.no_grad():
        for i, data in enumerate(test_loader, 0):
            Hnet.zero_grad()
            Rnet.zero_grad()
            all_pics = data  # allpics contains cover images and secret images
            this_batch_size = int(all_pics.size()[0] / 3)  # get true batch size of this step

            # first half of images will become cover images, the rest are treated as secret images
            cover_img = all_pics[0:this_batch_size, :, :, :]  # batchSize,3,256,256
            secret_img1 = all_pics[this_batch_size:this_batch_size * 2, :, :, :]
            secret_img2 = all_pics[this_batch_size * 2:this_batch_size * 3, :, :, :]
            secret_img = torch.cat([secret_img1, secret_img2], dim=1)
            ...
            Rlosses.update(errR.item(), this_batch_size)
            rev_secret_img1=rev_secret_img[:,0:3,:,:]
            rev_secret_img2 = rev_secret_img[:, 3:6, :, :]
            secret_img1=secret_img[:,0:3,:,:]
            secret_img2=secret_img[:, 3:6, :, :]
            #jisuan psnr
            ori_transfer = util.tensor2img(cover_img)  # uint8

            ori_secret1 = util.tensor2img(secret_img1)  # uint8
            ori_secret2 = util.tensor2img(secret_img2)
            result_transfer = util.tensor2img(container_img)  # uint8
            i_secret1 = util.tensor2img(rev_secret_img1)  # uint8
            i_secret2 = util.tensor2img(rev_secret_img2)
            ...
            psnr = util.calculate_psnr(ori_secret1, i_secret1)
            ssim = util.calculate_ssim(ori_secret1, i_secret1)
            test_results['psnr_secret1'].append(psnr)
            test_results['ssim_secret1'].append(ssim)

            psnr = util.calculate_psnr(ori_secret2, i_secret2)
            ssim = util.calculate_ssim(ori_secret2, i_secret2)
            test_results['psnr_secret2'].append(psnr)
            test_results['ssim_secret2'].append(ssim)
    ...
    ave_psnr_secret1 = sum(test_results['psnr_secret1']) / len(test_results['psnr_secret1'])
    ave_ssim_secret1 = sum(test_results['ssim_secret1']) / len(test_results['ssim_secret1'])

    ave_psnr_secret2 = sum(test_results['psnr_secret2']) / len(test_results['psnr_secret2'])
    ave_ssim_secret2 = sum(test_results['ssim_secret2']) / len(test_results['ssim_secret2'])
    print('ave_psnr:', ave_psnr)
    print('ave_ssim:', ave_ssim)
    print('ave_psnr_secret1:', ave_psnr_secret1)
    print('ave_ssim_secret1:', ave_ssim_secret1)

    print('ave_psnr_secret2:', ave_psnr_secret2)
    print('ave_ssim_secret2:', ave_ssim_secret2)
